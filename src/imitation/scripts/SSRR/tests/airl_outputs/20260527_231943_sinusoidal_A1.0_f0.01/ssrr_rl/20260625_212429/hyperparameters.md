# SSRR RL Run — 20260625_212429

## Lineage
- **AIRL run**: `20260527_231943_sinusoidal_A1.0_f0.01`
...
| `reg_weight_decay` | `0.001` | L2 regularisation |
| `reg_batch_size` | `64` | Snippet pairs per gradient step |
| `reg_n_steps` | `3000` | Total gradient steps per ensemble member |
| `reg_num_snippets` | `20000` | Total snippet examples drawn from the noisy-rollout buckets |

### SSRRRegressionConfig
| Parameter | Value | Notes |
|-----------|-------|-------|
| `min_steps` | `50` | Shortest snippet length sampled (steps) |
| `max_steps` | `150` | Longest snippet length sampled (steps) |
| `target_scale` | `5.0` | Scalar multiplied onto the sigmoid target before regression; larger values spread the loss landscape |
| `length_normalize` | `False` | If True, target is `sigma(eta) * (L / T)` — discounts shorter snippets proportionally to their length |
